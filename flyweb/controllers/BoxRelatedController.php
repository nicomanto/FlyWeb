...
class BoxRelatedController extends BaseController {
    ...
    public function get_RandomElements() {
        $query = "SELECT ID_Viaggio, Titolo, DescrizioneBreve, Immagine, AltImmagine FROM Viaggio WHERE DataInizio >= CURDATE() AND ID_Viaggio IN (SELECT ID_Viaggio FROM TagViaggio WHERE ID_Tag=? OR ID_Tag=? OR ID_Tag=? OR ID_Tag=?) AND ID_Viaggio!=? ORDER BY RAND() LIMIT ?";
